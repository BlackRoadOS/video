...
var worker_default = {
  async fetch(request, env) {
    ...
    if (path === "/api/health") {
      return jsonResp({ status: "ok", service: "roadview", version: "2.0.0", timestamp: (/* @__PURE__ */ new Date()).toISOString() });
    }
    ...
    if (wsMatch) {
      ...
      try {
        await env.DB.prepare("UPDATE video_rooms SET last_active = ? WHERE id = ?").bind((/* @__PURE__ */ new Date()).toISOString(), roomId).run();
      } catch {
      }
